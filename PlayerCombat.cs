...
// TO DO
// Grabbing, Cloud Bumping
// Nice to haves: Summon, Teleport, Vehicle Hops (Ride Clouds, smash into each other)
public class PlayerCombat : MonoBehaviour
{
    ...
    // Surfing variables
    [Header("Surfing Settings")]
    public KeyCode surfButton = KeyCode.C;
    public float surfSpeed = 15f;
    ...
    private float minWeight = 0.3f; // Minimum weight to avoid zero/negative
    ...
    void Update()
    {
        ...
        CheckGrounded();
        CheckSurfingInput();
        // Handle jump input for both normal and surfing states
            ...
            if (distance < attackRange)
            {
                ...
                if (!isGrounded)
                {
                    if (aerialHitCount < maxAerialCombo)
                    {
                        ...
                        // Lighter = higher boost, needs to be decreased
                        float adjustedBoost = aerialBoost / enemyWeight / 1.1f;
    ...
    IEnumerator BreakCube()
    {
        ...
        // Find all big clouds in the scene
        GameObject[] bigClouds = GameObject.FindGameObjectsWithTag("BigCloud");
        
        if (bigClouds.Length == 0)
        {
            Debug.Log("No big clouds to break!");
            isBreaking = false;
            yield break;
        }

        // Get the first big cloud
        GameObject bigCloud = bigClouds[0];
        Vector3 bigCloudPos = bigCloud.transform.position;
        Vector3 bigCloudScale = bigCloud.transform.localScale;

        // Calculate the size for the smaller clouds (about 1/2 of the big cloud)
        float smallCloudSize = bigCloudScale.x * 0.5f;
        Vector3 smallCloudScale = new Vector3(smallCloudSize, smallCloudSize, smallCloudSize);

        // Create three clouds in a triangle formation
        float radius = bigCloudScale.x * 0.5f; // Distance from center
        float angleStep = 360f / 3f; // Divide circle into 3 parts
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            Vector3 position = bigCloudPos + offset;
            GameObject smallCloud = Instantiate(cloudPrefab, position, Quaternion.identity);
            smallCloud.transform.localScale = smallCloudScale;

            // Add explosion force
            Rigidbody rb = smallCloud.GetComponent<Rigidbody>();
            if (rb != null)
            {
                Vector3 explosionDir = (position - bigCloudPos).normalized;
                rb.AddForce(explosionDir * breakForce, ForceMode.Impulse);
            }
        }

        // Destroy the big cloud
        Destroy(bigCloud);
    ...
    private void CheckGrounded()
    {
        ...
        // Alternative method using a sphere cast
        if (!isGrounded)
        {
    ...
    private void CheckSurfingInput()
    {
        if (Input.GetKeyDown(surfButton) && isGrounded && !isSurfing)
        {
            StartSurfing();
            Debug.Log("Started surfing");
        }
        else if (Input.GetKeyUp(surfButton) && isSurfing)
        {
            StopSurfing();
